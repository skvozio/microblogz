...
from app import db
from app.main.forms import EditProfileForm, PostForm, MessgeForm
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user == current_user:
            flash('You cannot follow yourself, loser!')
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            return redirect(url_for('main.index'))
        if user == current_user:
            flash('You cannot unfollow yourself, loser!')
            return redirect(url_for('main.user', username=username))
...
def search():
    ...
    next_url = url_for('main.search', q=g.search_for.q.data, page=page+1) \
        if total > page * current_app.config['POSTS_PER_PAGE'] else None
    prev_url = url_for('main.search', q=g.search_for.q.data, page=page-1) \
        if page > 1 else None
